...
public class SuvVehicleService implements VehicleService {
    ...
    @Autowired
    public SuvVehicleService(ParkingAreaRepository parkingAreaRepository, ParkRepository parkRepository,
    ...
    @Override
    public void checkOut(Vehicle vehicle,Integer parkingAreaId) throws VehicleNotFoundException {
        Park park = parkRepository.findByLicensePlateAndParkingAreaIdAndCheckOut(vehicle.getLicensePlate(),parkingAreaId,null);
        if (park == null)
            throw new VehicleNotFoundException(vehicle.getLicensePlate());
        ...
        Double fee = price.getHourlyPricing() * 1.1;
        park.setFee(fee);
